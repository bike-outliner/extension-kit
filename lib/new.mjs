...
export async function newExtension(id) {
  ...
  try {
    ...
    // Copy template directory, replacing {{id}} and {{api_version}} in
    // file contents. The kit's package version tracks Bike's extension
    // API version, so new extensions declare the API they were built for.
    const kitPackage = JSON.parse(
      await fs.readFile(path.join(packageRoot, 'package.json'), 'utf-8')
    )
    await copyTemplate(templateDir, dest, id, kitPackage.version)
...
async function copyTemplate(src, dest, id, apiVersion) {
  ...
  for (const entry of await fs.readdir(src, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      await copyTemplate(srcPath, destPath, id, apiVersion)
    } else {
    ...
    JSON.stringify(
      {
        ...
        // Lib check off: an extension project can't fix third-party
        // declarations, and the kit lib-checks its own api/*.d.ts itself.
        compilerOptions: { skipLibCheck: true },
        include: [
          '../src/*/dom/protocols.ts',
          '../src/*/app/**/*',
          '../src/*/tests/**/*',
          '../node_modules/@bike-outliner/extension-kit/api/app/globals.d.ts',
