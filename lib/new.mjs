...
`,
  ...
  tests: {
    ...
    content: (id) => `describe("${id}", () => {
  it("activates", () => {
    assert(true, "Extension should activate")
  })
})
`,
  },
...
`,
  ...
  'dom-protocols': {
    path: 'dom/protocols.ts',
    content: `import { DOMProtocol } from 'bike/core'
...
export async function newExtension(id) {
  ...
  try {
    ...
    await fs.writeFile(path.join(dest, 'CHANGELOG.md'), `# Changelog\n\n## 0.1.0 (In Progress)\n`, 'utf-8')
    ...
    if (contexts.includes('app')) {
      const stub = STUBS.tests
      const filePath = path.join(dest, stub.path)
      await fs.mkdir(path.dirname(filePath), { recursive: true })
      await fs.writeFile(filePath, stub.content(id), 'utf-8')
    }

    if (contexts.includes('app') && contexts.includes('dom')) {
      const stub = STUBS['dom-protocols']
      const filePath = path.join(dest, stub.path)
      await fs.writeFile(filePath, stub.content, 'utf-8')
    }
