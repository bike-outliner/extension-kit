...
export function copyAndValidatePlugin(outdir, srcDir, validateManifest, validateTheme, { manifestPattern, themePattern }) {
  return {
    ...
    setup(build) {
      ...
      build.onResolve({ filter: /copy-and-validate$/ }, (args) => ({
        path: args.path,
      ...
      build.onEnd(async () => {
        ...
        for (const file of themes) {
          validateJsonFile(file, validateTheme, 'Theme', { allowComments: true })
          const dest = path.join(outdir, path.relative(srcDir, file))
          fs.mkdirSync(path.dirname(dest), { recursive: true })
          fs.copyFileSync(file, dest)
        }

        // Copy resources/ directories from extensions
        const resourceDirs = await fastGlob(path.join(srcDir, '**/resources'), { onlyDirectories: true })
        for (const dir of resourceDirs) {
          const dest = path.join(outdir, path.relative(srcDir, dir))
        ...
        const virtualMap = virtualOutput + '.map'
        if (fs.existsSync(virtualMap)) fs.unlinkSync(virtualMap)
...
function validateJsonFile(file, validate, label, { allowComments = false } = {}) {
  const raw = fs.readFileSync(file, 'utf-8')
  const content = allowComments ? stripJsonComments(raw) : raw
  let data
  try {
...
// Strip `//` and `/* ... */` comments from JSON-with-comments. Markers inside
// string literals are preserved; newlines from block comments are kept so that
// JSON.parse error line numbers still match the source file.
function stripJsonComments(text) {
  let out = ''
  let mode = 'code' // 'code' | 'string' | 'line' | 'block'
