...
export function copyAndValidatePlugin(outdir, srcDir, validateTheme, { manifestPattern, themePattern, apiThemePattern }) {
  return {
    ...
    setup(build) {
      build.onEnd(async () => {
        ...
        // Validate API theme files
        const apiThemes = await fastGlob(apiThemePattern)
        for (const file of apiThemes) {
          validateThemeFile(file, validateTheme)
        }
      })
    },
