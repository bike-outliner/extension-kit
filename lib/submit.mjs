...
export function submitExtension(id) {
  ...
  try {
    ...
    // Validate the updated list against the registry's schema before
    // opening a PR, so malformed entries are caught locally.
    const schemaPath = join(tmpDir, 'extensions.schema.json')
    if (existsSync(schemaPath)) {
      const ajv = new Ajv({ allErrors: true })
      addFormats(ajv)
      const validate = ajv.compile(JSON.parse(readFileSync(schemaPath, 'utf8')))
      if (!validate(extensions)) {
        console.error('Error: registry entry failed schema validation:')
